...
export const generateChatCompletion = async (req, res, next) => {
    ...
    try {
        ...
        if (!user)
            return res.status(401).json({ message: "User not recognized or Token Malfunctioned" }); //You should store these messages inside constant.ts so you can access them over and over again
        //grab chats of the user to get the context of the conversation and send them along with the new one
        const chats = user.chats.map(({ role, content }) => ({ role, content })); //get all existing chats
